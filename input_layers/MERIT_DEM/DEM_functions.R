...
saga_DEM_derivatives <- function(INPUT, MASK=NULL, sel=c("SLP","CPR","TWI","CRV","VBF","VDP","OPN","DVM","MRN","TPI"), RADIUS=c(9,13), cpus=24){
  ...
  ## Uplslope curvature:
  if(any(sel %in% "CRV")){
    if(!(file.exists(gsub(".sgrd", "_downlocal.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_downlocal.tif", INPUT)))){
  ...
  ## Slope:
  if(any(sel %in% "SLP")){
    if(!(file.exists(gsub(".sgrd", "_slope.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_slope.tif", INPUT)))){
      try( suppressWarnings( system(paste0('saga_cmd -c=', cpus,' ta_morphometry 0 -ELEVATION=\"', INPUT, '\" -SLOPE=\"', gsub(".sgrd", "_slope.sgrd", INPUT), '\"') ) ) )
    }
  }
  ## CProf:
  if(any(sel %in% "CPR")){
    if(!(file.exists(gsub(".sgrd", "_cprof.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_cprof.tif", INPUT)))){
      try( suppressWarnings( system(paste0('saga_cmd -c=', cpus,' ta_morphometry 0 -ELEVATION=\"', INPUT, '\" -C_PROF=\"', gsub(".sgrd", "_cprof.sgrd", INPUT), '\"') ) ) )
    }
  }
  ## MrVBF:
  if(any(sel %in% "VBF")){
    if(!(file.exists(gsub(".sgrd", "_vbf.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_vbf.tif", INPUT)))){
  ...
  ## Valley depth:
  if(any(sel %in% "VDP")){
    if(!(file.exists(gsub(".sgrd", "_vdepth.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_vdepth.tif", INPUT)))){
  ...
  ## Openess:
  if(any(sel %in% "OPN")){
    if(!(file.exists(gsub(".sgrd", "_openp.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_openp.tif", INPUT)))){
  ...
  if(any(sel %in% "DVM")){
    if(!(file.exists(gsub(".sgrd", "_devmean.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_devmean.tif", INPUT)))){
    ...
    if(!(file.exists(gsub(".sgrd", "_devmean2.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_devmean2.tif", INPUT)))){
  ...
  ## TWI:
  if(any(sel %in% "TWI")){
    if(!(file.exists(gsub(".sgrd", "_twi.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_twi.tif", INPUT)))){
  ...
  if(any(sel %in% "MRN")){
    if(!(file.exists(gsub(".sgrd", "_mrn.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_mrn.tif", INPUT)))){
  ...
  ## TPI:
  if(any(sel %in% "TPI")){
    if(!(file.exists(gsub(".sgrd", "_tpi.sgrd", INPUT)) | file.exists(gsub(".sgrd", "_tpi.tif", INPUT)))){
